...
Users of NEA typically want to test if there is a fuctional association between a query set of genes and a pathway\cite{alexeyenko2012network}. We argue that such test are much easier implemented as tests of the query set, i.e. is the query set a randomly selected set of genes, or is there an overrepresentation of well-connected genes in the query set?

Hence, here we employ a null model of the form ``The query genes are randomly selected the genome``. We present a dynamic programming algorithm, that we dubbed GeneSetDP, that calculates the exact score distribution of any query of a given size. We also implemented one random sampling algoritm for the same task, that we called GeneSetMC.
...
Source code of an implementation of GeneSetDP and GeneSetMC  as well as to the experiments conducte here in, is available from \url{https://github.com/statisticalbiotechnology/genesetdp}.
...
\begin{figure}[htb]
	\begin{center}
    \begin{tabular}{c@{}c@{}c}
      \includegraphics[width=5cm]{figures/score_distribuition.png} &
      \includegraphics[width=5cm]{figures/score_distribuition.png} &
      \includegraphics[width=5cm]{figures/score_distribuition.png} \\
      $Q=10$ & $Q=20$ & $Q=30$ \\
    \end{tabular}
	\end{center}
  {\em FIXME: update these figures.}
...
In order to test the statistical calibration of our method, we calculated $p$~values for 10000 random picks of gene sets from the investigated genome. As our selection is random and following our null hypothesis, we expected the resulting $p$~values to be uniformly distributed. To test this we plotted the $p$~values against their quantile in Figure \ref{fig:calibration} for $10000$ randomly assembeled queries from the human genome. As a comparison, we also added a calibration curve for the popular NEA method BinoX\cite{ogris2016novel}.
...
\begin{figure}[htb]
  \begin{center}
    \begin{tabular}{c@{}c@{}c}
      \includegraphics[width=5cm]{figures/calibration.png} &
      \includegraphics[width=5cm]{figures/calibration.png} &
      \includegraphics[width=5cm]{figures/calibration.png} \\
      $Q=10$ & $Q=20$ & $Q=30$
    \end{tabular}
...
Here we have implemented two methods, GeneSetDP and GeneSetMC, to calculate unbiased $p$~values for inferences from NEA. We demonstrated that the method gave more accurate statistics than the popular BinoX method using random subsets of a genome.
