...
\begin{flushleft}
{\Large
\textbf\newline{One exact and one random sampling method to assess the reliability of inferences from network enrichment analysis} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
To test the consistency of the methods, we also investigated the correspondance between the $p$~values reported by GeneSetDP and GeneSetMC. As can be seen in Fig~\ref{fig:pscatter}, the relative difference is larger for lower $p$~values, likely due to the sampling errors of GeneSetMC.

\begin{figure}[htb]
  \begin{center}
		  \begin{tabular}[t]{c}
        FIXME: Make this scatterplot in log-log, GeneSetDP on x-axis, GeneSetMC on y-axis.
        (Idea from reviewer comment in email 2018-06-08.)
%				\includegraphics[width=1.0\textwidth]{FIXME}
		\end{tabular}
  \end{center}
  \caption{{\bf Correspondance between GeneSetDP's and GeneSetMC's $p$~values.} We selected 10000 random sets of 10, 20 and 30 genes from the investigated genome definition and calculated their $p$ values with GeneSetMC and GeneSetDP against the ``Glycolysis/Gluconeogenesis'' pathway. We subsequently plotted the obtained $p$~values against each other .}
  \label{fig:pscatter}
\end{figure}
