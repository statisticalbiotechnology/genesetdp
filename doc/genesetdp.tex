...
We also downloaded the NEA example given at the BinoX web site. The example files include a pathway definition file that groups $6819$ genes into $289$ human pathways, and a network definition file that, after thresholding with a score of $0.7$, yeilds $1244992$ links between those genes.
...
It should be noted that our outlined method a score distribution could work well with Monte Carlo-sampling as well, by sampling random gene sets of size $Q$, which would be faster and easier to understand then the theory of how to permute networks.
